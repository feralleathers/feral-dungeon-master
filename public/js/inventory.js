...
const STORAGE_KEY = 'fdm.inventory.items.v2';
const PROFILE_KEY = 'fdm.inventory.profileName';
...
function setupProfileFields() {
  if (dom.profName) {
    dom.profName.value = safeLocalStorage('get', PROFILE_KEY) || '';
  }
  dom.saveProf?.addEventListener('click', () => {
    safeLocalStorage('set', PROFILE_KEY, dom.profName?.value.trim() || '');
...
function loadInventoryFromStorage() {
  const payload = safeReadJSON(STORAGE_KEY);
  let items = [];
  if (Array.isArray(payload)) items = payload;
  else if (payload && Array.isArray(payload.items)) items = payload.items;
  inventory = (items.length ? items : DEMO_ITEMS).map(normaliseItem).filter(Boolean);
  sortInventory();
}
...
function exportJSON() {
  const payload = JSON.stringify(
    { profileName: dom.profName?.value || '', items: inventory },
    null,
    2
  );
  download('fdm-inventory.json', payload, 'application/json');
  track('inventory:export', { format: 'json', count: inventory.length });
}
...
function persistInventory() {
  safeLocalStorage('set', STORAGE_KEY, JSON.stringify({ items: inventory }));
}
