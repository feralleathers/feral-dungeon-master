...
import { loadQuestionBank, persistQuestionBank } from '../question-bank.js';
...
const qbCategorySelect = document.getElementById('questionBankCategory');
const qbOptionsContainer = document.getElementById('questionBankOptions');
const qbAddButton = document.getElementById('qbAddOption');
const qbSaveButton = document.getElementById('qbSaveBank');
...
let qbActiveCategoryId = questionBank[0]?.id || null;
...
qbCategorySelect?.addEventListener('change', (evt) => {
  qbActiveCategoryId = evt.target.value;
  renderQuestionBankOptions();
});
qbAddButton?.addEventListener('click', () => {
  const category = questionBank.find((entry) => entry.id === qbActiveCategoryId);
  if (!category) return;
  category.options.push('');
  renderQuestionBankOptions();
});
qbSaveButton?.addEventListener('click', handleQuestionBankSave);
qbOptionsContainer?.addEventListener('click', (evt) => {
  const target = evt.target.closest('[data-remove-option]');
  if (!target) return;
  const category = questionBank.find((entry) => entry.id === qbActiveCategoryId);
  if (!category) return;
  const index = Number(target.dataset.removeOption);
...
function renderQuestionBankControls() {
  renderQuestionBankCategories();
  renderQuestionBankOptions();
}

function renderQuestionBankCategories() {
  if (!qbCategorySelect) return;
  qbCategorySelect.innerHTML = questionBank.map((category) => `<option value="${category.id}">${category.label}</option>`).join('');
  if (!questionBank.some((category) => category.id === qbActiveCategoryId)) {
    qbActiveCategoryId = questionBank[0]?.id || null;
  }
  qbCategorySelect.value = qbActiveCategoryId || '';
}

function renderQuestionBankOptions() {
  if (!qbOptionsContainer) return;
  const category = questionBank.find((entry) => entry.id === qbActiveCategoryId);
  if (!category) {
    qbOptionsContainer.innerHTML = '<p class="small">Select a category to edit.</p>';
    return;
  }
  if (!category.options.length) {
    qbOptionsContainer.innerHTML = '<p class="small">No options yet. Add one then save.</p>';
    updateQuestionBankStatus(`Editing ${category.label}.`);
    return;
  }
  qbOptionsContainer.innerHTML = category.options.map((option, idx) => `
    <div class="question-bank-option">
      <input type="text" data-option-index="${idx}" value="${escapeHtml(option)}">
      <button class="btn" data-remove-option="${idx}">Remove</button>
    </div>
  `).join('');
  updateQuestionBankStatus(`Editing ${category.label}.`);
}

function handleQuestionBankSave() {
  const category = questionBank.find((entry) => entry.id === qbActiveCategoryId);
  if (!category || !qbOptionsContainer) return;
  const inputs = qbOptionsContainer.querySelectorAll('input[data-option-index]');
  const values = Array.from(inputs)
    .map((input) => input.value.trim())
    .filter(Boolean);
  category.options = values;
  persistQuestionBank(questionBank);
  updateQuestionBankStatus(`${category.label} saved.`, 'success');
  renderQuestionBankOptions();
}

function updateQuestionBankStatus(message, tone = 'info') {
  if (!qbStatus) return;
  qbStatus.textContent = message;
  qbStatus.dataset.tone = tone;
}
